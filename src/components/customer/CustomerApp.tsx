...
export const CustomerApp: React.FC = () => {
  ...
  const appContent = (
    <div className="relative flex flex-col flex-1 h-full w-full bg-[#FFF8F0] dark:bg-[#211713] text-[#2B1A15] dark:text-[#FAF4EE] font-sans overflow-hidden">
      ...
      {/* Dynamic Festival / Promotion Popup Modal */}
      {settings.popupEnabled && !promoDismissed && (
        <div
          id="festival-promo-popup-modal"
          className="absolute inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-xs animate-fadeIn"
        >
          <div className="relative w-full max-w-sm bg-white dark:bg-[#30221D] rounded-3xl overflow-hidden shadow-2xl border border-[#E8DACD] dark:border-[#46332B]">
            {/* Close 'X' Button */}
            <button
              id="close-promo-popup-btn"
              onClick={() => setPromoDismissed(true)}
              className="absolute top-3 right-3 z-20 w-8 h-8 rounded-full bg-black/50 hover:bg-black/80 text-white flex items-center justify-center backdrop-blur-md transition"
            >
              <X className="w-4 h-4" />
            </button>
            ...
            {settings.popupImageUrl && (
              <div className="relative h-44 w-full overflow-hidden bg-[#FDF2E4]">
                <img
                ...
                <div className="absolute bottom-3 left-3 right-3 text-white">
                  <span className="text-[10px] uppercase font-black tracking-widest px-2 py-0.5 rounded-full bg-[#C9A227] text-[#2B1A15] inline-block mb-1">
                    Special Highlight
                  </span>
            ...
            <div className="p-4 space-y-3 text-[#2B1A15] dark:text-[#FAF4EE]">
              {!settings.popupImageUrl && (
                <h3 className="text-lg font-black text-[#8B2F3C] dark:text-[#C9A227]">
                  {settings.popupTitle}
                </h3>
              )}
              ...
              <button
                id="promo-popup-action-btn"
                onClick={() => {
                  setPromoDismissed(true);
                  if (settings.popupButtonAction === 'custom-cake') {
                    setCustomCakeModalOpen(true);
                  } else if (settings.popupButtonAction === 'categories') {
                    setCustomerTab('categories');
                  } else if (settings.popupButtonAction === 'offers') {
                    setCustomerTab('home');
                  }
                }}
                className="w-full py-3 rounded-2xl bg-[#8B2F3C] hover:bg-[#742531] text-white font-bold text-xs uppercase tracking-wider shadow-lg shadow-[#8B2F3C]/25 active:scale-[0.98] transition flex items-center justify-center gap-2 border border-[#C9A227]/40"
              >
                <span>{settings.popupButtonText || 'Explore Now'}</span>
              </button>
            </div>
          </div>
        </div>
